    ...
    class NetSimScenarioManet
    {
        ...
        static void Main(string[] args)
        {
            ...
            if (args.Length == 6)
            {
                ...
                simulation_time = float.Parse(args[5]);
            }
            else
            {
                Console.WriteLine("Incorrect!! Required 6 arguments\n");
                return;
            }
            ...
            for (i = 0; i < total_node; i++)
            {
                ...
                string max = addNet.next_mac(mac);
            ...
            else
            {
                for (i = 0; i < max_application; i++)
                {
                    ...
                    application[i].DESTINATION_ID = rnd.Next(1, total_node);
                    application[i].ID = i + 1;
                    application[i].NAME = Convert.ToString(i + 1);
                    int source;
                    while ((source = rnd.Next(1, total_node)) == application[i].DESTINATION_ID) ;
