...
namespace NetSim_Scenario_Generation_Internetworks
{
    ...
    public class NetSimScenario
    {
        ...
        static void Main(string[] args)
        {
            ...
            if (args.Length == 8)
            {
                max_interface = Int32.Parse(args[0]);
                max_hop = Int32.Parse(args[1]);
                max_node = Int32.Parse(args[2]);
                max_application = Int32.Parse(args[3]);
                config_helper_location = args[4];
                exp_name = args[5];
                version_name = args[6];
           
                simulation_time = float.Parse(args[7]);
            }
            else
            {
                Console.WriteLine("Incorrect!! Required 8 arguments\n");
                return;
            }

            Console.WriteLine(max_interface);
            Console.WriteLine(max_hop);
            ...
            if (max_application > max_node * (max_node - 1) / 2)
            {
                Console.WriteLine("Max application cannot be greater than max_node * (max_node - 1)/2");
                return;
            }
            ...
            //int max_interface = 20;   /* max upto 25 */
            //int max_hop = 6;          /* should be 2,4,6(even)*/
            //int max_node = 25;        /*max upto Math.Pow(max_interface-1,max_hop/2)*(max_interface-1) */
            ...
            XmlNode root = nsWriter.open_document(config_helper_location + "\\ConfigHelper\\header.txt");
            ...
            int source = max_router + 1;
            int destination = source + 1;

            for (i = 0; i < max_application; i++)
            {
                if (destination == total_node + 1)
                {
                    source = source + 1;
                    destination = source + 1;
                }

                application[i].DESTINATION_ID = destination;
            ...
            nsWriter.add_experimentInfo(root, exp_name, version_name);
            //nsWriter.add_element_from_file(root, config_helper_location + "\\ConfigHelper\\experimentInfo.txt");
            nsWriter.add_element_from_file(root, config_helper_location + "\\ConfigHelper\\guiInfo.txt");
            addNet.add_network(root, total_node, link_count, max_application, device_container, link, application, config_helper_location);
