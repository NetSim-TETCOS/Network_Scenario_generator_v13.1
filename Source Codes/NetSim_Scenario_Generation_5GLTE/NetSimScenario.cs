...
namespace NetSim_Scenario_Generation_5GLTE
{
    ...
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            double gnb_x0 = 300.0, gnb_y0 = 300.0;
            double gnb_radius = 0.0;
            ...
            for (; i < (max_router + max_node + max_gnb); i++)
            {
               ...
               // epc_last_interface = device_container[total_device - 1].device.INTERFACE_COUNT;
                ...
                device_container[i].pos_3d.X_OR_LON =  gnb_x0+gnb_radius * Math.Cos(Math.PI * (theta / 180.0));
                device_container[i].pos_3d.Y_OR_LAT =  gnb_y0+gnb_radius * Math.Sin(Math.PI * (theta / 180.0));
                ...
                for (int l = 0; l < num_ue_per_gnb; l++)
                {
                    ...
                    device_container[ue_id - 1].pos_3d.X_OR_LON = gnb_x0 + ue_radius * Math.Cos(Math.PI * (theta1 / 180.0));
                    device_container[ue_id - 1].pos_3d.Y_OR_LAT = gnb_y0 + ue_radius * Math.Sin(Math.PI * (theta1 / 180.0));
