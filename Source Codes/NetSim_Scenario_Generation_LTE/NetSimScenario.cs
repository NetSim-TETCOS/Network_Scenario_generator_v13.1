...
namespace NetSim_Scenario_Generation_5GLTE
{
    ...
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            //int max_enb = 5;             //(between 1-98)
            //int max_ue = 20;              //(between 1-10000 and multiple of max_enb)
            ...
            double router_x0 = 100.0, router_y0 = 200.0, offset_radius = 60.0;
            /********************************************************************************************/

            /********************************************************************************************/
            //fixed epc , enb  && ue info
            int max__epc_interface = max_enb + 1;
            double epc_x0 = 100.0, epc_y0 = 100.0;
            double enb_x0 = 300.0, enb_y0 = 300.0;
            double enb_radius = 0.0;
            double enb_angle_increment = 360.0 / max_enb;
            double ue_angle_increment = 360.0 / max_ue;
            double ue_radius = 100.0;
            int num_ue_per_enb = (max_ue / max_enb);
            /********************************************************************************************/


            /********************************************************************************************/
            //intermediate parameters used in generating scenario
            int total_device = max_router + max_node + max_enb + max_ue + max_epc;
            int max_link = max_router + max_node + max_enb + max_ue + 100;
            ...
            /********************************************************************************************/
            // this block initialises set of independed attributtes of EPC, router and wirednode

            device_container[0].device.DEVICE_ID = 1;
            device_container[0].device.DEVICE_NAME = "EPC_" + Convert.ToString(1);
            device_container[0].device.DEVICE_TYPE = "MME";
            device_container[0].device.INTERFACE_COUNT = 0;

            device_container[0].pos_3d.X_OR_LON = epc_x0;
            device_container[0].pos_3d.Y_OR_LAT = epc_y0;

            device_container[0]._interface = new INTERFACE[max__epc_interface];
            ...
            for (; i <= (max_router + max_node + max_enb); i++)
            {

               // epc_last_interface = device_container[total_device - 1].device.INTERFACE_COUNT;
                device_container[0]._interface[epc_last_interface].ID = epc_last_interface + 1;
                device_container[0]._interface[epc_last_interface].DEFAULT_GATEWAY = "";
                device_container[0]._interface[epc_last_interface].SUBNET_MASK = addNet.subnet_mask();
                string ip = addNet.next_ip(++ipv4_count, 1);
                device_container[0]._interface[epc_last_interface].IP_ADDRESS = ip;
                mac = addNet.next_mac(mac);
                device_container[0]._interface[epc_last_interface].MAC_ADDRESS = mac;
                device_container[0]._interface[epc_last_interface].CONNECTED_TO = "eNB_" + Convert.ToString(i + 1);
                device_container[0]._interface[epc_last_interface].INTERFACE_TYPE = "LTE";
                ...
                device_container[i].device.DEVICE_ID = i + 1;
                device_container[i].device.DEVICE_NAME = "eNB_" + Convert.ToString(i + 1);
                device_container[i].device.DEVICE_TYPE = "LTE_eNB";
                device_container[i].device.INTERFACE_COUNT = 0;
                ...
                device_container[i]._interface[0].INTERFACE_TYPE = "LTE_S1";

                device_container[i]._interface[1].ID = 2;
                mac = addNet.next_mac(mac);
                device_container[i]._interface[1].MAC_ADDRESS = mac;
                device_container[i]._interface[1].INTERFACE_TYPE = "LTE";


                double theta = i * enb_angle_increment;
                device_container[i].pos_3d.X_OR_LON =  enb_x0+enb_radius * Math.Cos(Math.PI * (theta / 180.0));
                device_container[i].pos_3d.Y_OR_LAT =  enb_y0+enb_radius * Math.Sin(Math.PI * (theta / 180.0));
                double low_angle = theta - (num_ue_per_enb * ue_angle_increment / 2.0);
                ...
                link[link_count].link_device = new LINK_DEVICE[num_ue_per_enb + 1];
                ...
                for (int l = 0; l < num_ue_per_enb; l++)
                {
                    int ue_id = k + max_router + max_node + max_enb + max_epc;
                    if (ue_id >= total_device + 1)
                        break;

                    device_container[ue_id - 1].device.DEVICE_ID = ue_id;
                    device_container[ue_id - 1].device.DEVICE_NAME = "UE_" + Convert.ToString(ue_id );
                    device_container[ue_id - 1].device.DEVICE_TYPE = "LTE_UE";
                    ...
                    device_container[ue_id - 1].pos_3d.X_OR_LON = enb_x0 + ue_radius * Math.Cos(Math.PI * (theta1 / 180.0));
                    device_container[ue_id - 1].pos_3d.Y_OR_LAT = enb_y0 + ue_radius * Math.Sin(Math.PI * (theta1 / 180.0));
                    ...
                    device_container[ue_id - 1]._interface[last_interface].CONNECTED_TO = "eNB_" + Convert.ToString(i + 1);
                    ...
                    device_container[ue_id - 1]._interface[last_interface].INTERFACE_TYPE = "LTE";
                ...
                device_container[0].device.INTERFACE_COUNT = epc_last_interface + 1;
                epc_last_interface++;
            }
            /**********************************************************************************************/
            ...
            device_container[1].pos_3d.X_OR_LON = router_x0;
            device_container[1].pos_3d.Y_OR_LAT = router_y0;

            epc_last_interface = device_container[0].device.INTERFACE_COUNT;
            device_container[0]._interface[epc_last_interface].ID = epc_last_interface + 1;
            device_container[0]._interface[epc_last_interface].DEFAULT_GATEWAY = "";
            device_container[0]._interface[epc_last_interface].IP_ADDRESS = addNet.next_ip_1(ipv4_count, 1);
            device_container[0]._interface[epc_last_interface].SUBNET_MASK = addNet.subnet_mask();
            mac = addNet.next_mac(mac);
            device_container[0]._interface[epc_last_interface].MAC_ADDRESS = mac;
            device_container[0]._interface[epc_last_interface].INTERFACE_TYPE = "WAN";
            device_container[0]._interface[epc_last_interface].CONNECTED_TO = "Router_" + Convert.ToString(max_router + 1);
            device_container[0].device.INTERFACE_COUNT = epc_last_interface + 1;



            device_container[1]._interface[0].ID = 1;
            device_container[1]._interface[0].DEFAULT_GATEWAY = "";
            device_container[1]._interface[0].IP_ADDRESS = addNet.next_ip_1(ipv4_count, 2);
            device_container[1]._interface[0].SUBNET_MASK = addNet.subnet_mask();
            mac = addNet.next_mac(mac);
            device_container[1]._interface[0].MAC_ADDRESS = mac;
            device_container[1]._interface[0].INTERFACE_TYPE = "WAN";
            device_container[1]._interface[0].CONNECTED_TO = "EPC_1";
            device_container[1].device.INTERFACE_COUNT = max_node + max_epc;
            ...
            link[link_count].link_device[1].INTERFACE_ID = 1;
            link[link_count].link_device[1].NAME = "Router_" + Convert.ToString(2);
            link[link_count].link_device[0].DEVICE_ID = 1;
            link[link_count].link_device[0].INTERFACE_ID = epc_last_interface + 1;
            link[link_count].link_device[0].NAME = "EPC_" + Convert.ToString(1);
            link_count++;
            ...
            device_container[1].level.r = 0.0;
            double theta0 = device_container[1].level.theta = 0.0;
            double level0 = device_container[1].level.level = 0;
            double increment0 = device_container[1].level.increment = (360.0 / Math.Pow(branch, level0 + 1));
            device_container[1].level.low_angle = theta0 - (branch * 1.0) * increment0 / 2.0;
            device_container[1].pos_3d.X_OR_LON = router_x0;
            device_container[1].pos_3d.Y_OR_LAT = router_y0;
            ...
            while (max_node_temp != 0)
            {
                ...
                {
                    int router_id = j + 1;
                    int id = device_container[i + 1].device.DEVICE_ID;
                    ...
                    string ip = addNet.next_ip_2(ipv4_count, 1);
                    ...
                    device_container[router_id - 1]._interface[router_last_interface - 1].CONNECTED_TO = "Wired_Node_" + Convert.ToString(id);
                    ...
                    device_container[id - 1]._interface[last_interface].DEFAULT_GATEWAY = addNet.next_ip_2(ipv4_count, 1);
                    device_container[id - 1]._interface[last_interface].IP_ADDRESS = addNet.next_ip_2(ipv4_count, 2);
                    mac = addNet.next_mac(mac);
                    ...
                    device_container[id - 1]._interface[last_interface].CONNECTED_TO = "Router_" + Convert.ToString(router_id);
                    ...
                    device_container[router_id - 1].device.INTERFACE_COUNT = router_last_interface;
            ...
            else
            {
                ...
                //destination = max_router + max_node + max_enb + 1 to max_router + max_node + max_enb + max_ue
                int node_start = max_router + max_epc + 1;
                int node_end = max_epc+max_router + max_node + 1;
                int ue_start = max_epc+max_router + max_node + max_enb + 1;
                //int ue_end = max_router + max_node + max_enb + max_ue + 1;
                int node = node_start;
