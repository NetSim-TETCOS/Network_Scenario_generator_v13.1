...
namespace NetSim_Scenario_Generation_5GLTE
{
    class AddNetworkElement
    {
        ...
        */
        ...
        /*
          func: adds a ENB(device) as child node to DEVICE_CONFIGUARTION(parent node)
          params: deviceConfig:-reference to parent node(DEVICE_CONFIGUARTION)
                  device_attribute:- attributs of ENB
                  pos_3d_attribute:-postion info of ENB
                  interface_variables:- interface info of ENB
          return: doesnt return
        */
        public void add_ENB(XmlNode deviceConfig, DEVICE device_attribute, POS_3D pos_3d_attribute, INTERFACE[] interface_variables, string config_helper_location)
        {
            XmlNode device = nsWriter.add_element_from_file_with_format(deviceConfig, config_helper_location + "\\ConfigHelper\\ENB\\Device.txt",
                  Convert.ToString(device_attribute.DEVICE_ID),
                  Convert.ToString(device_attribute.DEVICE_NAME),
                  Convert.ToString(device_attribute.INTERFACE_COUNT),
                  Convert.ToString(pos_3d_attribute.X_OR_LON),
            ...
            for (int i = 0; i < device_attribute.INTERFACE_COUNT; i++)
            {
                if (interface_variables[i].INTERFACE_TYPE.Equals("LTE"))
                {
                    nsWriter.add_element_from_file_with_format(device, config_helper_location + "\\ConfigHelper\\ENB\\LTE\\Interface.txt",
                         Convert.ToString(interface_variables[i].ID),
                         Convert.ToString(interface_variables[i].MAC_ADDRESS));
                ...
                {
                    nsWriter.add_element_from_file_with_format(device, config_helper_location + "\\ConfigHelper\\ENB\\LTE_S1\\Interface.txt",
                         Convert.ToString(interface_variables[i].ID),
                         Convert.ToString(interface_variables[i].MAC_ADDRESS));

                }
            }
            nsWriter.add_element_from_file(device, config_helper_location + "\\ConfigHelper\\ENB\\Application_Layer.txt");
        ...
        */
        public void add_deviceConfig(XmlNode parent, int device_count, DEVICE_CONTAINER[] device_container, string config_helper_location)
        {
            ...
            for (int i = 0; i < deviceCount; i++)
            {
                if (device_container[i].device.DEVICE_TYPE.Equals("ROUTER"))
                ...
                else if (device_container[i].device.DEVICE_TYPE.Equals("LTE_UE"))
                    ...
                    add_ENB(deviceConfig, device_container[i].device, device_container[i].pos_3d, device_container[i]._interface, config_helper_location);
        ...
        */
        public void add_connection(XmlNode parent, int link_count, LINK[] link, string config_helper_location)
        {
            ...
            for (int i = 0; i < link_count; i++)
            {
                ...
                if (link[i].link_type.Equals("ENB_UE"))
                {
                    link_element = nsWriter.add_element_from_file_with_format(con, config_helper_location + "\\ConfigHelper\\Link\\ENB_UE\\Link.txt",
                    ...
                    nsWriter.add_element_from_file_with_format(link_element, config_helper_location + "\\ConfigHelper\\Link\\ENB_UE\\Medium_Property.txt");
                }
                else if(link[i].link_type.Equals("ENB_EPC"))
                {
                    link_element = nsWriter.add_element_from_file_with_format(con, config_helper_location + "\\ConfigHelper\\Link\\ENB_EPC\\Link.txt",
                    Convert.ToString(link[i].DEVICE_COUNT),
                    Convert.ToString(link[i].LINK_ID),
                    Convert.ToString(link[i].LINK_NAME));
                    for (int j = 0; j < link[i].DEVICE_COUNT; j++)
                    ...
                    nsWriter.add_element_from_file_with_format(link_element, config_helper_location + "\\ConfigHelper\\Link\\ENB_EPC\\Medium_Property.txt");
